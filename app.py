...
def prediccion():
    ...
    return render_template('index.html', prediccion=f'El análisis de la predicción es: {p_s}', num_correos=f'{correos}')
...
def dibuja_grafico():
    ...
    plt.title('Análisis de correos Spam y Normal')
    ax = plt.subplot()          
    ax.set_xticks(x_values)            
    ax.set_xticklabels(x_values)        
    ax.set_xlabel('Tipo de correo 0:Normal y 1:Spam')  
    ax.set_ylabel('Cantidad o Proporción de correos') 
    output = io.BytesIO()
    plt.legend()
